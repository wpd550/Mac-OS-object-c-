...
- (void)didSelectObjectCount:(void (^)(NSInteger, NSInteger, NSInteger))selectCountBlock {
    NSInteger totalCount = self.baseViewModel.selectedObjects.count;
    NSInteger leafCount = 0;
    NSInteger internalCount = 0;
    for (id node in self.baseViewModel.selectedObjects) {
        if ([node isLeaf]) {
            leafCount += 1;
        } else {
            internalCount += 1;
    ...
    if (selectCountBlock != nil) {
        selectCountBlock(totalCount, leafCount, internalCount);
    }
}
